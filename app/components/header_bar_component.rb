...
class HeaderBarComponent < ApplicationComponent
  ...
  renders_one :breadcrumb_collection, -> (collection) { BreadcrumbComponent.with_collection(collection) }
  renders_one :icon_link_collection, ->(collection) { 
    IconLinkComponent.with_collection(collection, class: "bg-secondary p-2 self-start rounded text-white") 
  }
