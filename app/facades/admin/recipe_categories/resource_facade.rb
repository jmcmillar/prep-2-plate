class Admin::RecipeCategories::ResourceFacade
  ...
  def id
    ["recipe_category", @resource.id].join("_")
  end
  ...
  def action_turbo_data
    TurboData[
      :table_actions,
      [:table_actions, label: @resource.name, id: @resource.id, controller_path: "admin/recipe_categories"]
    ]
  end
end
