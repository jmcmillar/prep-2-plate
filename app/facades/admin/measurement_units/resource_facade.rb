class Admin::MeasurementUnits::ResourceFacade
  ...
  def id
    ["measurement_unit", @resource.id].join("_")
  end
  ...
  def action_turbo_data
    TurboData[
      :table_actions,
      [:table_actions, label: @resource.name, id: @resource.id, controller_path: "admin/measurement_units"]
    ]
  end
end
