class Admin::UserRecipes::ResourceFacade
  ...
  def id
    ["user_recipe", @resource.id].join("_")
  end
  ...
  def action_turbo_data
    TurboData[
      :table_actions,
      [:table_actions, label: @resource.name, id: @resource.id, controller_path: "admin/user_recipes"]
    ]
  end
