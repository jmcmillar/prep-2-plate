  ...
  def build_records
    attribute_data_array.each do |attribute_data|
      set_ingredient(attribute_data).recipe_ingredients.new(
        ...
        denominator: Recipe::ParseQuantity.new(attribute_data.quantity).to_value.denominator,
        numerator: Recipe::ParseQuantity.new(attribute_data.quantity).to_value.numerator,
